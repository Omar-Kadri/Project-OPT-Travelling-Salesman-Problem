...
def TSP(matrice):
 ...
 while listP:
   niv = listP[-1][0]
   #Check if 1 city left or if 1 city of this current level left
   if ((listP.__len__()<2) or (niv != listP[-2][0])):
    actif = listP.pop()[1]
   #Reorder cities of current level
   else :
    list2 = []
    list2.append(listP.pop())
    #Put all cities of current level in list2
   ...
   for j in range(N):
     if (actif.matrice[i][j] != math.inf):
        ...
        listP.append((fils.niv,fils,matrice[i][j]))
 if not solution:
